<?php
/*
./assets/variablesSelectOptions.tpl
./assets/jsLangs.tpl
./assets/newVariable.tpl
./assets/admin/create.tpl
./assets/admin/list.tpl
./assets/admin/settings.tpl
./assets/admin/edit.tpl*/

_("Error");
_("Variable is not removed");
_("Message");
_("Variable successfully removed");
_("Error");
_("Variable is not updated");
_("Message");
_("Variable successfully updated");
_("Error");
_("Variable is not added");
_("Message");
_("Variable successfully added");
_("Message");
_("Variable must be surrounded by $");
_("Message");
_("Variable must have a value");
_("Creating mail template");
_("Go back");
_("Save");
_("Save and go back");
_("Settings");
_("Template name (only latin)");
_("From");
_("From email");
_("Theme");
_("Message type");
_("Template user mail");
_("Send email to user");
_("Yes");
_("No");
_("Admin mail template");
_("Send email to admin");
_("Yes");
_("No");
_("Admin address");
_("Template description");
_("Template email deleting");
_("Delete selected templates");
_("Delete");
_("Cancel");
_("Template list view");
_("Back");
_("Settings");
_("Delete");
_("Create template");
_("Name");
_("Description");
_("Theme");
_("Templates list empty");
_("Settings");
_("Back");
_("Save");
_("Settings");
_("Install standart templates");
_("From");
_("From email");
_("Admin email");
_("Theme");
_("Use wraper");
_("Wraper");
_("Server path to sendmail");
_("Protocol");
_("Port");
_("Check email sending");
_("Email template editing");
_("Go back");
_("Save");
_("Save and go back");
_("Template settings");
_("Template variables");
_("Settings");
_("Template name(only latin)");
_("From");
_("From email");
_("Theme");
_("Message type");
_("Template user mail");
_("Send email to user");
_("Yes");
_("No");
_("Admin mail template");
_("Send email to admin");
_("Yes");
_("No");
_("Admin address");
_("Template description");
_("Variables");
_("Values");
_("Edit");
_("Delete");
_("Add new variable");
_("From");

?>