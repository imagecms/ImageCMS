...
function showMessage($message, $title = FALSE, $class = '', $ret = false) {
    ...
    if ($title == FALSE) {
        $title = lang('Message', 'admin') . ": ";
        if ($class == 'r') {
            $title = lang('Error', 'admin') . ": ";
        }
        if ($class == 'g') {
            $title = lang('Success', 'admin') . ": ";
        }
    }
    $CI = & get_instance();
    $message .= '<br/><strong>' . lang('Requests to the database', 'admin') . ': ' . $CI->db->total_queries() . '</strong>';
    $message = str_replace("\n", '<br/>', $message);
...
function ajax_redirect($location) {
    echo lang('Redirecting', 'admin') . ': <b>' . $location . '</b> ' . "<script type='text/javascript'> setTimeout(\"location.href = '" . $location . "';\",3000); </script>";
}
