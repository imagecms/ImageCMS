...
<div class="m-t_20">
    <form method="get">
        ...
        <div style='width: 150px;'>
            <select name='show_by'>
                <option value="Count" {if $show_by == 'Count'} selected="selected" {/if}>{lang('Count', 'mod_stats')}</option>
                <option value="Price" {if $show_by == 'Price'} selected="selected" {/if}>{lang('Price', 'mod_stats')}</option>
            </select>
        </div>
    </form>
</div>
