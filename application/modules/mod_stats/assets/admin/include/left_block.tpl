<div class="span3">
    ...
    <ul class="nav nav-tabs nav-stacked">
        ...
        <li>
            ...
            <ul class="submenu second-level-ul" style="{if !strpos($_SERVER['REQUEST_URI'], implode("/", array($firstLevel.controller,'')))}display: none;{/if}">
                {foreach $firstLevel.items as $secondLevel}
                <li>
                    <a href="{site_url('admin/components/cp/mod_stats/')}/{$secondLevel.controller}/{$secondLevel.action}{$queryString}" class="linkChart {if strpos($_SERVER['REQUEST_URI'], implode("/", array($secondLevel.controller,$secondLevel.action)))} active{/if}">
                        <span class="simple_tree">↳</span>
                        <span class="text-el">{$secondLevel.name}</span>
                    </a>
                </li>
                {/foreach}
            </ul>
