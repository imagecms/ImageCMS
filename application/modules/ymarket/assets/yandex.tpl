...
<yml_catalog date="{echo date('Y-m-d H:i')}">
    <shop>
        ...
        <currencies>
            <currency id="{echo $infoXml['mainCurr']['code']}" rate="{echo $infoXml['mainCurr']['rate']}"/>            
        ...
        <offers>
            {foreach $infoXml['offers'] as $id => $offer}
                <offer 
                    id="{echo $id}" 
                    {if $offer['cpa'] > 0 } available="true" {else:} available="false" {/if}
                    {if $full} quantity="{$offer['quantity']}"{/if}
                    >            
