                        ...
                        <div class="inside_padd discount-out">
                            <div class="form-horizontal">
                                ...
                                <div class="noLimitC">
                                    ...
                                    <div class="span8">
                                        {$isGiftSertificate = ($discount['type_discount'] == 'certificate');}
                                        {if $discount['max_apply'] != null && $discount['max_apply'] != '0'}
                                            {$maxApply = true;}
                                        {/if}

                                        <span class="d-i_b m-r_10" > 
                                            <input class="input-small onlyNumbersInput " id="how-much" type="text" name="max_apply"{if $maxApply}value="{echo $discount['max_apply']}"{/if} {if !$maxApply || $isGiftSertificate}  disabled="disabled" {/if} maxlength="7"/>
                                        </span>
                                        <span class="d-i_b v-a_m" >
                                            <span {if $isGiftSertificate}style="display: none"{/if} class="frame_label no_connection m-r_15 spanForNoLimit spanForNoLimitCheckbox" >
                                    ...
                                    <div class="span6">
                                        ...
                                        <div class="">
                                            ...
                                            <div id="all_orderBlock" class="forHide" {if $discount['type_discount'] != 'all_order'}style="display: none;{/if}">
                                                <span class="d_b m-b_10">
                                                    <span class="d-i_b sum-of-order">
                                                        <input class="input-small onlyNumbersInput" type="text" name="all_order[begin_value]" value="{if $discount['all_order']}{echo $discount['all_order']['begin_value']}{else:}0{/if}" maxlength="9" /></span>
                                                    <span class="d-i_b">{echo $CS}</span>
                                                </span>
                                                <div class="m-b_5">
                                                    <span class="frame_label no_connection m-r_15 spanForNoLimit noLimitChecker" >
                                                        <span class="niceCheck" style="background-position: -46px 0px; ">
                                                            <input type="checkbox" {if $discount['all_order']['for_autorized']}checked="checked"{/if} name="all_order[for_autorized]" value="1" class="noLimitCountCheck">
                                                        </span>
                                            ...
                                            {foreach $userGroups as $group}
                                                <span class="frame_label">
                                                    <span class="niceRadio b_n">
                                                        <input type="radio" name="group_user[group_id]" {$checked} value="{echo $group[id]}" {if $group[id] == $discount['group_user']['group_id']}checked=checked{/if} />
                                                    </span>
                                                    {echo $group['alt_name']}
                                                </span>
                                                {$checked=''}
                                        ...
                                        <div id="categoryBlock" class="forHide" {if $discount['type_discount'] != 'category'} style="display: none;" {/if}>
                                            <select name="category[category_id]">
                                                {foreach $categories as $category}
                                                    <option {if $category->getLevel() == 0}style="font-weight: bold;"{/if} value="{echo $category->getId()}" {if $category->getId() == $discount['category']['category_id']}selected=selected{/if}>{str_repeat('-',$category->getLevel())}{echo $category->getName()}</option>
                                                {/foreach}
                                            </select></br>
                                            <span class="frame_label" >
                                                <span class="niceCheck">
                                                    <input type="checkbox" name="category[child]" value="1"{if $discount['category']['child']}checked="checked"{/if}/>
                                                </span>
                                                {lang('Change child category', 'mod_discount')}
                                            </span>
                                        </div>
