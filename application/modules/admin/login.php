...
class Login extends BaseAdminController {
    ...
    function do_login() {
        ...
        $this->form_validation->set_rules('password', lang("Password"), 'trim|required|min_length[5]|max_length[32]');
...
//				$remember = true;
        ...
        if ($this->dx_auth->is_max_login_attempts_exceeded()) {
            $this->form_validation->set_rules('captcha', lang("Protection code"), 'trim|required|xss_clean|callback_captcha_check');
        }
...
//			    if ($this->form_validation->run($this) == TRUE)
            ...
            if ($rezult == TRUE) {
                $this->lib_admin->log(lang("Entered the IP control panel") . $this->input->ip_address());

                redirect('admin/admin/init', 'refresh');
            } else {
                $this->template->assign('login_failed', lang("Username and password have not been found"));
            }
        }
...
//			$this->template->show('login', TRUE);
    ...
    function forgot_password() {
        ...
        $val->set_rules('login', lang("Username or Email"), 'trim|required|xss_clean');

        // Validate rules and call forgot password function
        if ($val->run() AND $this->dx_auth->forgot_password($this->input->post('login'))) {
            $this->template->assign('info_message', '<div class="alert alert-info">' . lang("Please check your email for instructions on how to activate the new password.") . '</div>');
        }
    ...
    function captcha_check($code) {
        ...
        if ($this->dx_auth->is_captcha_expired()) {
            $this->form_validation->set_message('captcha_check', lang("Wrong protection code"));
//			$result = FALSE;
            $result = TRUE;
