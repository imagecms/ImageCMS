<!--<div class="top-navigation">
            ...
            <li>
                <form id="image_upload_form" style="width:100%;" method="post" enctype="multipart/form-data" action="{site_url('admin/components/run/gallery/upload_image/' . $album['id'])}">
                    ...
                    <div style="height:16px;width:130px;float:left;text-align: center; overflow: hidden;" class="button_silver_130">
                        <div style="color:#000000;">{lang("Select a file")}</div>
                    ...
                    <input type="submit" name="action" value="{lang("Upload a file")}" class="button_silver_130" />
    ...
    <div align="right" style="padding:7px 13px;">
        <a href="#" onclick="ajax_div('page', base_url + 'admin/components/cp/gallery/'); return false;"  >{lang("Gallery")}</a> 
        >
...
<section class="mini-layout">
    <div class="frame_title clearfix">
        <div class="pull-left">
            ...
            <span class="title w-s_n">{lang("Album")}: <a href="/admin/components/cp/gallery/edit_album/{$album.id}" class="pjax">{$album.name}</a></span>
        </div>
    ...
    <div class="row inside_padd">
        ...
        <div class="span7">
            <dl class="dl-horizontal m-t_20">
                ...
                <dt>{lang("Has been downloaded or downloaded")}</dt>
                <dd>{date('Y-m-d H:i', $image.uploaded)}</dd>
                <dt>{lang("Views")}</dt>
                <dd>{$image.views}</dd>
                <dt>{lang("File size")}</dt>
            ...
            <form method="post" action="{site_url('admin/components/run/gallery/update_info/' . $image.id)}" id="change_img_desc" class="form-horizontal">
                ...
                <label class="number">
                    {lang("Position")}<input type="text" value="{$image.position}" name="position" />
                </label>
                <label>
                    {lang("Description")}
                ...
                <div class="m-t_10">
                    <button type="submit" class="btn btn-primary formSubmit" data-form="#change_img_desc"><i class="icon-ok"></i> {lang("Save")}</button>
                    <a href="/admin/components/cp/gallery/edit_album/{$album.id}" class="btn">{lang("Cancel")}</a>
                </div>
                {form_csrf()}
            </form>

            <form method="post" action="{site_url('admin/components/cp/gallery/rename_image/' . $image.id)}" id="change_imn_name_form">
                <label>{lang("New name")}<input type="text" class="textbox_short" name="new_name" /></label>
                <button type="submit" class="btn btn-primary formSubmit" data-form="#change_imn_name_form"><i class="icon-ok"></i> {lang("Save")}</button>
                {form_csrf()}
            </form>
        </div>
    </div>
