<section class="mini-layout">
    <div class="frame_title clearfix">
        <div class="pull-left">
            ...
            <span class="title w-s_n">{lang("Album")}: {$album['name']}</span>
        </div>
        <div class="pull-right">
            <div class="d-i_b">
                ...
                <button type="button" class="btn btn-small btn-primary formSubmit" data-form="#addPics" data-submit><i class="icon-white icon-ok"></i>{lang("Save")}</button>
                <button type="button" class="btn btn-small btn-danger action_on disabled"  onclick="$('.modal').modal('show');"><i class="icon-trash icon-white"></i>{lang("Delete")}</button>
            </div>
        </div>                            
    </div>  
                                            ...
                                            <td>
                                                ...
                                                <div class="t-a_c photo_album o_h">
                                                    ...
                                                    <div class="btn-group f-s_0">
                                                        <button type="button" class="btn" data-rel="tooltip" onclick="shopCategories.deleteCategoriesConfirm($(this).closest('td').find('[name=ids]').val());" data-title="{lang("Delete")}" data-remove=""><i class="icon-remove"></i></button>
                                                ...
                                                <div class="m-t_10">
                                                    <b>{lang("Name")}:</b> <span title="{$item.file_name}{$item.file_ext}">{truncate($item['file_name'], 10)}{$item.file_ext}</span><br/>
                                                    <b>{lang("Size")}:</b> {$item.file_size} Kb
                                                </div>
...
<div class="modal hide fade">
    <div class="modal-header">
        ...
        <h3>{lang("Deleting photos")}:</h3>
    </div>
    <div class="modal-footer">
        <a href="#" class="btn" onclick="$('.modal').modal('hide');">{lang("Cancel")}</a>
        <a href="#" class="btn btn-primary" onclick="shopCategories.deleteCategoriesConfirm()">{lang("Delete")}</a>
    </div>
</div>