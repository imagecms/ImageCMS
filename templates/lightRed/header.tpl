                ...
                <div class="phones-header">
                    ...
                    <div class="btn-order-call">
                        <button data-tab="true" data-href="#ordercall" data-drop="#ordercall" data-source="{site_url('shop/callback')}">
                            ...
                            <span class="text-el d_l">{lang('Заказать звонок','lightRed')}</span>
                        </button>
                    </div>
                </div>
                <div class="head-work">
                    <span class="c_b">{lang('Работаем', 'lightRed')}:</span> <span class="s-t">{lang('Пн–Пт', 'lightRed')} 09:00–20:00,<br/>{lang('Сб', 'lightRed')} 09:00–17:00, {lang('Вс выходной', 'lightRed')}</span>
                </div>
                <!--                End. Contacts block-->
                        ...
                        <div class="p_r">
                            <form name="search" method="get" action="{shop_url('search')}">
                                <span class="btn-search">
                                    <button type="submit"><span class="icon_search"></span><span class="text-el">{lang('Найти','lightRed')}</span></button>
                                </span>
                                <div class="frame-search-input">
                                    <input type="text" class="input-search" id="inputString" name="text" autocomplete="off" value="{if strpos($CI->uri->uri_string, 'search') !== false}{htmlspecialchars($_GET['text'])}{/if}"  placeholder="{lang('Я ищу', 'lightRed')}&hellip;"/>
