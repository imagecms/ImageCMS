    ...
    <div class="container">
        <div class="f-s_0 title-cart without-crumbs">
            <div class="frame-title">
                <h1 class="title">{lang('Список желаний','lightRed')}</h1>
            </div>
        </div>
        {if $errors}
        ...
        <ul class="tabs tabs-wishlist" data-cookie="wishlistTabs">
            <li {if $_COOKIE['wishlistTabs'] == "#list-products"}class="active"{/if}>
                <button type="button" data-href="#list-products">
                    <span class="text-el d_l_1">{lang('Список товаров','lightRed')}</span>
                </button>
            </li>
            <li {if $_COOKIE['wishlistTabs'] == "#data-users"}class="active"{/if}>
                <button type="button" data-href="#data-users">
                    <span class="text-el d_l_1">{lang('Дополнительные данные','lightRed')}</span>
                </button>
            </li>
        </ul>
            ...
            <div id="list-products" {if $_COOKIE['wishlistTabs'] == "#list-products"}style="display: block;"{/if}>
                <div class="frame-button-add-wish-list">
                    <div class="btn-cart">
                        <button type="button" data-drop=".drop-add-wishlist" data-place="inherit" data-overlay-opacity="0" data-effect-on="slideDown" data-effect-off="slideUp">
                            ...
                            <span class="text-el">{lang('Создать новый список','lightRed')}</span>
                        </button>
                    </div>
                    <span class="help-block">{lang('В список избранных вы можете отложить понравившиеся товары, также показать список друзьям', 'lightRed')}</span>
                </div>
                <div class="drop drop-style-2 drop-add-wishlist">
                    <div class="drop-header">
                        <div class="title">{lang('Создание списка избранных товаров','lightRed')}</div>
                    </div>
                            ...
                            <div class="horizontal-form big-title">
                                <form method="POST" action="{site_url('/wishlist/wishlistApi/createWishList')}">
                                    ...
                                    <div class="frame-label">
                                        <span class="title">{lang('Доступность:','lightRed')}</span>
                                        <div class="frame-form-field check-public">
                                            <div class="lineForm">
                                                <select name="wlTypes" id="wlTypes">
                                                    <option value="shared">{lang('Коллективный', 'lightRed')}</option>
                                                    <option value="public">{lang('Публичный', 'lightRed')}</option>
                                                    <option value="private">{lang('Приватный', 'lightRed')}</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    <label>
                                        <span class="title">{lang('Название списка:','lightRed')}</span>
                                        <span class="frame-form-field">
                                            <input type="text" value="" name="wishListName"/>
                                        </span>
                                    </label>
                                    <label>
                                        <span class="title">{lang('Описание:','lightRed')}</span>
                                        <span class="frame-form-field">
                                    ...
                                    {else:}
                                        <div class="msg layout-highlight layout-highlight-msg">
                                            <div class="info">
                                                ...
                                                <span class="text-el">{lang('Список пуст','lightRed')}</span>
                                            </div>
                                        </div>
                                    {/if}
                                </div>
                                    ...
                                    <div class="f_l">
                                        ...
                                        <div>
                                            {if $wishlist[0]['access'] == 'shared' || $wishlist[0]['access'] == 'public'}
                                                <div class="btn-buy btn-send-wishlist">
                ...
                {else:}
                    <div class="msg layout-highlight layout-highlight-msg">
                        <div class="info">
                            <span class="icon_info"></span>
                            <span class="text-el">{lang('Список Желания пуст','lightRed')}</span>
                        </div>
                    </div>
                {/if}
            </div>
                            ...
                            <form action="{site_url('/wishlist/do_upload')}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                                ...
                                <div class="photo-block m-b_5">
                                    ...
                                    <span id="wishlistphoto" data-width="{echo $settings[maxImageWidth]}" data-height="{echo $settings[maxImageHeight]}" data-src="{$THEME}{$colorScheme}/images/nophoto.png">
                                        {if $user['user_image']!=''}
                                            <img src="{site_url('uploads/mod_wishlist/'.$user['user_image'])}" alt="{lang('Фото пользователя', 'lightRed')}"/>
                                        {else:}
                                    ...
                                    <div class="group-buttons f-s_0">
                                        <label class="btn-edit-photo-wishlist" for="img">
                                            <button type="button" class="p_r hidden">
                                                ...
                                                <span class="text-el">{lang('Выбрать', 'lightRed')}</span>
                                            </button>
                                        </label>
                                        {if $user['user_image']!=''}
                                            <div class="btn-remove-photo-wishlist">
                                                <button
                                                    ...
                                                    >
                                                    <span class="icon_remove"></span>
                                                    <span class="text-el">{lang('Удалить', 'lightRed')}</span>
                                                </button>
                                            </div>
                                        {/if}
                                    </div>
                                ...
                                <div class="btn-def download-btn disabled">
                                    <input type="submit" value="{lang('загрузить катринку','lightRed')}" data-wishlist="do_upload" disabled="disabled"/>
                                </div>
                                {form_csrf()}
                            </form>
                        </div>
                ...
                <div class="right-wishlist-data">
                    {/*}<div class="btn-edit-WL">
                        <button type="button" data-drop=".form-data" data-place="inherit"  class="d_l_1">
                            <span class="text-el">{lang('Редактировать','lightRed')}</span>
                        </button>
                    </div>
                    { */}
                    <div class="form-data horizontal-form big-title">
                        <form>
                            <input type="hidden" value="{echo $user[id]}" name="user_id"/>
                            <label>
                                <span class="title">{lang('ФИО:','lightRed')}</span>
                                <span class="frame-form-field">
                                    <input type="text" value="{echo $user[user_name]}" name="user_name"/>
                                </span>
                            </label>
                            <label>
                                <span class="title">{lang('Дата рождения:','lightRed')}</span>
                                <span class="frame-form-field">
                            ...
                            <label>
                                <span class="title">{lang('Дополнительно:','lightRed')}</span>
                                <span class="frame-form-field">
                                ...
                                <div class="frame-form-field">
                                    <div class="btn-def">
                                        <button
                                            ...
                                            >
                                            <span class="text-el">{lang('Сохранить','lightRed')}</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
