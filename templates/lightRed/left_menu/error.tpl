...
{/literal}
<div class="error">
	<h3 align="center">{lang('Внимание','lightRed')}!</h3>
	<p>{lang('Не обнаружен необходимый файл шаблона для меню ','lightRed')} <b>{$menu}</b></p>
	<ul>
	{foreach $errors as $e}
	...
	<p>
		{if $tpl_folder}
		{lang('Проверьте наличие файла или измените настройки меню','lightRed')} <b>{$menu}</b>
		{else:}
		{lang('Переустановите модуль меню ','lightRed')}
		{/if}
	</p>
</div>	