...
<div class="frame-bask frameBask p_r">
    ...
    {if $totalItems > 0}
        <div class="drop-header">
            <div class="title bask">{lang('Мой заказ','lightRed')}</div>
        </div>
        <div class="drop-content">
            <div class="inside-padd">
                <table class="table-order">
                    <thead>
                        <tr>
                            <th colspan="2">{lang('Товар', 'lightRed')}</th>
                            <th>{lang('Кол-во', 'lightRed')}</th>
                            <th>{lang('Cумма', 'lightRed')}</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            {if $item->instance === "SProducts"}
                                <tr data-id="{echo $item->getId()}" class="items items-bask cart-product">
                                        ...
                                        <div class="description">
                                            {if $item->getName() && trim($item->getName()) != trim($item->getSProducts()->getName())}
                                                <span class="frame-variant-name">
                                                    <span class="text-el">{lang('Вариант','lightRed')}</span>
                                                    <span class="code">({echo trim($item->getName())})</span>
                                                </span>
                                            {/if}
                                            {if $item->getNumber()}
                                                <span class="frame-variant-code">{lang('Артикул','lightRed')}  
                                    ...
                                    <td class="frame-count frameCount">
                                        <div class="number js-number" data-title="{lang('Количество на складе','lightRed')} {echo $item->getStock()}">
                                            <input type="text" value="{echo $item->quantity}" class="plusMinus plus-minus" id="inputChange{echo $item->getId()}" data-id="{echo $item->getId()}" data-title="{lang('Только цифры','lightRed')}" data-min="1" data-max="{echo $item->getStock()}"/>
                                        </div>
                                    </td>
                            ...
                            {else:}
                                <tr class="row-kits" data-id="{echo $item->getId()}">
                                                        ...
                                                        <div class="description">
                                                            ...
                                                            {if $kitItem->getSProducts()->getNumber()}
                                                                <span class="frame-variant-code">
                                                                    <span class="text-el">{lang('Артикул','lightRed')}</span>
                                    ...
                                    <td class="frame-count">
                                        <div class="number js-number" data-title="{lang('Количество на складе','lightRed')} {echo $item->getStock()}">
                                            <input type="text" value="{echo $item->quantity}" class="plusMinus plus-minus" id="inputChange{echo $item->getId()}" data-id="{echo $item->getId()}" data-kit="1" data-title="{lang('Только цифры','lightRed')}" data-min="1" data-max="{echo $item->getStock()}"/>
                                        </div>
                                    </td>
                                    <td class="frame-cur-sum-price">
                                        <div class="frame-prices f-s_0">
                ...
                <div class="clearfix">
                    ...
                    <div class="btn-form f_l isCart">
                        <button type="button" data-closed="closed-js">
                            <span class="text-el"><span class="f-s_14">←</span> {lang('Вернуться к оформлению','lightRed')}</span>
                        </button>
                    </div>
                    <span class="s-t">{lang('Всего','lightRed')}:</span>
                ...
                <div class="clearfix inside-padd">
                    <div class="btn-form f_l">
                        <button type="button" data-closed="closed-js">
                            ...
                            <span class="text-el"><span class="f-s_14">←</span> {lang('Вернуться к покупкам','lightRed')}</span>
                        </button>
                    </div>
                    <div class="btn-buy btn-buy-p f_r">
                        <a href="{shop_url('cart')}">
                            <span class="icon_cart_p"></span>
                            <span class="text-el">{lang('Оформить заказ','lightRed')}</span>
                        </a>
                    </div>
                </div>
            </div>
    ...
    {else:}
        <div class="drop-header">
            <div class="title">{lang('Ваша корзина','lightRed')} <span class="add-info">{lang('пуста','lightRed')}</span></div>
        </div>
        <div class="drop-content is-empty">
            <div class="inside-padd">
                <div class="msg f-s_0">
                    <div class="success"><span class="icon_info"></span><span class="text-el">{lang('Вы удалили все элементы из корзины','lightRed')}</span></div>
                </div>
