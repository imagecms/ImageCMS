...
(function() {
    ...
    if (!c.localStorage) {
        if (c.globalStorage) {
            try {
                c.localStorage = c.globalStorage
            } catch (f) {
            }
            return
        }
        var g = document.createElement("div"), d = "localStorage";
        ...
        if (g.addBehavior) {
            ...
            var b = c.localStorage = {length: 0, setItem: function(e, h) {
                    ...
                    g.load(d);
                    var e = 0;
                    while (attr = g.XMLDocument.documentElement.attributes[e++]) {
                        g.removeAttribute(attr.name)
                    }
                    g.save(d);
                    this.length = 0
                }, key: function(e) {
