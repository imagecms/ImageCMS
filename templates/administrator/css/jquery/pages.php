	...
	public function add()
	{
		...
		}else
        {
			...
			if($this->input->post('page_url') == '' or $this->input->post('page_url') == NULL)
			{
				...
				if($this->input->post('prev_text')){
					$translit_obuv = $this->input->post('page_title').'-'.$this->input->post('prev_text');
				}else{
					$translit_obuv = $this->input->post('page_title');
				}
				
				if($this->input->post('category') != 0) $url = translit_url($translit_obuv);
				else $url = translit_url($this->input->post('page_title'));
