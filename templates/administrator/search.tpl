    <section class="mini-layout">
        ...
        <div class="row-fluid">
            ...
            <div class="clearfix">
                <div class="btn-group myTab m-t_20 pull-left" data-toggle="buttons-radio">
                    <a href="#pages" class="btn btn-small {if count($pages)} active{/if}">{lang("Pages")} 
                        <span style="top:-13px;" class="badge {if count($pages)}badge-important{/if}">{count($pages)}</span>
                    </a>
                    <a href="#users" class="btn btn-small {if !count($pages) && count($users)} active{/if}">{lang("User")}
	    ...
	    {else:}
                <div class="alert alert-info" style="margin: 18px;">{lang("No relevant data has been found")}</div>
            {/if}   
            </div>
