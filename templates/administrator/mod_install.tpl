<div id="mod-tabs-block">
    ...
    <div>
        ...
        <p style="padding:5px;">
            <b>{lang("Author")}:</b> {$module.author}
            <br />
            <b>{lang("Version")}:</b> {$module.version}
        </p>

        <div align="center" style="padding:5px;">
        {if $install_type == 'ftp'}
            <input type="button" value="{lang("Install")}" class="button_silver" onclick="show_connection_block();" />
        {else:}
            <input type="button" value="{lang("Install")}" class="button_silver" onclick="ajax_request('{$BASE_URL}admin/mod_search/connect_ftp/{$module.id}');" />
        {/if}
            
            <a href="{$module.file}" target="_blank">{lang("Download")}</a>
        </div>
...
<form method="post" action="{$BASE_URL}admin/mod_search/connect_ftp/{$module.id}" id="connect_ftp_form" style="width:100%;" >
		...
		<div class="form_text"></div>
		<div class="form_input">
               <input type="submit" value="{lang("Next")}" class="button_silver" onclick="ajax_me('connect_ftp_form');" /> 
        </div>
		<div class="form_overflow"></div>
</form>
{/if}
