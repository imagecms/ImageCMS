            ...
            <form method="post" id="form_change_info" onsubmit="ImageCMSApi.formAction('{site_url("/shop/profileapi/changeInfo")}', '#form_change_info', {literal}{hideForm: false, durationHideForm: 1000}{/literal});
                ...
                <label>
                    ...
                    <span class="frame-form-field">
                        ...
                        <span class="help-block">{lang('Не меньше 4-х символов','greyVision')}</span>
                    </span>
                </label>
                <label>
                ...
                <label>
                    ...
                    <span class="frame-form-field">
                        ...
                        <span class="help-block">{lang('E-mail является логином','greyVision')}</span>
                    </span>
                </label>
                ...
                <label>
                    <span class="title">{lang('Адрес','greyVision')}:</span>
                    <span class="frame-form-field">
                ...
                <div class="frame-label">
                    ...
                    <span class="frame-form-field">
                        <span class="btn-form">
                            <input type="submit" value="{lang('Сохранить данные','greyVision')}"/>
                        </span>
                    </span>
                </div>
                {form_csrf()}
    ...
    {if $dApi->discountsExists()}
        ...
        <div class="layout-highlight info-discount">
            <div class="title-default">
                <div class="title">{lang('Скидки','greyVision')}</div>
            </div>
            <div class="content">
                <ul class="items items-info-discount">
                    <li class="inside-padd">
                        <div>
                            {lang('Товаров на сумму','greyVision')}:
                        ...
                        {if $dApi->userDiscountExists()}
                            <div>
                                {lang('Ваша текущая скидка','greyVision')}:
                                <span class="price-item">
                        ...
                        {if $dApi->groupDiscountExists()}
                            <div>
                                {lang('Ваша текущая скидка группы пользователей','greyVision')}:
                                <span class="price-item">
                    ...
                    {if $discount_comul_next}
                        <li class="inside-padd">
                            <div>{lang('Для следующей скидки','greyVision')} {echo $discount_comul_next['value']}{if  $discount_comul_next['type_value'] == 1}%{else:}{$CS}{/if}</b> {lang('осталось','greyVision')}</div>
                            <div>{lang('cделать покупки на сумму','greyVision')}: <b>{echo $discount_comul_next['begin_value'] - $profile->getamout()} {$CS}</b></div>
                        </li>
                    {/if}
                    {if  $discount['comulativ']}
                        <li class="inside-padd">
                            <button type="button" class="d_l_1" data-drop=".drop-comulativ-discounts" data-place="noinherit" data-placement="top left" data-overlay-opacity= "0">{lang('Посмотреть таблицу скидок','greyVision')}</button>
                        </li>
                    {/if}
                </ul>
            </div>
...
{if  $discount['comulativ']}
    <div class="drop-style drop drop-comulativ-discounts">
        ...
        <div class="drop-header">
            <div class="title">{lang('Накопительные скидки','greyVision')}</div>
        </div>
        <div class="drop-content">
            <div class="inside-padd characteristic">
                <table class="">
                    <thead>
                        <tr>
                            <th>{lang('Размер скидки','greyVision')}</th>
                            <th>{lang('От','greyVision')}</th>
                            <th>{lang('До','greyVision')}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {foreach $discount['comulativ'] as $disc}
                            <tr>
                                ...
                                <td>{if $disc['end_value']}{echo $disc['end_value']} {$CS}{else:}{lang('Бесконечно','greyVision')}{/if}</td>
                            </tr>
                        {/foreach}
                    </tbody>
                </table>
