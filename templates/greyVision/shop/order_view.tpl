    ...
    <div class="container">
        {if $CI->session->flashdata('makeOrderForTpl') === true}
            <div class="f-s_0 without-crumbs">
                <div class="frame-title">
                    <h1 class="title">{lang('Спасибо, ваш заказ принят!', 'greyVision')}<br/>{lang('Наши менеджеры свяжутся с вами.','greyVision')}</h1>
                </div>
            </div>
        {/if}
        <div class="f-s_0 title-order-view without-crumbs">
            <div class="frame-title">
                <h1 class="title">{lang('Заказ №','greyVision')}:<span class="number-order">{echo $model->getId()}</span></h1>
            </div>
        </div>
        ...
        <div class="left-order">
            ...
            <table class="table-info-order">
                ...
                <tr>
                    <th>{lang('Имя получателя','greyVision')}:</th>
                    <td>{echo $model->getUserFullName()}</td>
                </tr>
                {if $model->getUserPhone()}
                    <tr>
                        <th>{lang('Телефон','greyVision')}:</th>
                ...
                {if $s_field.field_data && $s_field.field_data !== ''}
                    <tr>
                        <th>{lang('Дополнительный телефон','greyVision')}:</th>
                ...
                {if $model->getDeliveryMethod()}
                    ...
                    <tr>
                        <th>{lang('Способ доставки','greyVision')}:</th>
                        <td>
                ...
                {if $s_field.field_data && $s_field.field_data !== ''}
                    <tr>
                        <th>{lang('Город','greyVision')}:</th>
                        <td>{echo $s_field.field_data}</td>
                    </tr>
                {/if}
                {if $model->getUserDeliverTo()}
                    <tr>
                        <th>{lang('Адрес','greyVision')}:</th>
                        <td>{echo $model->getUserDeliverTo()}</td>
                    </tr>
                {/if}
                {if $model->getUserComment()}
                    <tr>
                        <th>{lang('Комментарий','greyVision')}:</th>
                ...
                <tr>
                    <th>{lang('Дата заказа','greyVision')}:</th>
                ...
                {if $paymentMethod}
                    <tr>
                        <th>{lang('Способ оплаты','greyVision')}:</th>
                        <td>
                ...
                <!--Start. Order status-->
                <tr>
                    <th>{lang('Статус оплаты','greyVision')}:</th>
                    <td>
                        {if $model->getPaid() == true}
                            <span class="status-pay paid">{lang('Оплачен','greyVision')}</span>
                        {else:}
                            <span class="status-pay not-paid">{lang('Не оплачен','greyVision')}</span>
                        {/if}
                    </td>
                </tr>
                <!--End. Order status-->
                        ...
                        <table class="table-order table-order-view">
                            ...
                            <tbody>
                                ...
                                {foreach $model->getOrderProducts() as $orderProduct}
                                    ...
                                    <tr class="items items-bask items-order cart-product items-product">
                                        <td class="frame-items">
                                            ...
                                            <div class="description">
                                                <span class="frame-variant-name-code">
                                                    {if trim(ShopCore::encode($orderProduct->variant_name) != '')}<span class="frame-variant-name frameVariantName">{lang("Вариант",'greyVision')}: <span class="code js-code">{echo ShopCore::encode($orderProduct->variant_name)}</span></span>{/if}
                                                    {if trim(ShopCore::encode($Variant->getNumber()) != '')}<span class="frame-variant-code frameVariantCode">{lang("Артикул",'greyVision')}: <span class="code js-code">{echo ShopCore::encode($Variant->getNumber())}</span></span>{/if}
                                                </span>
                                                {/*}
                                        ...
                                        <td>
                                            <div class="count-or-compl">{lang('Кол-во','greyVision')}:</div>
                                            <span class="plus-minus">{echo $orderProduct->getQuantity()}</span>
                                            <span class="text-el">{lang('шт','greyVision')}.</span>
                                        </td>
                                ...
                                {foreach $model->getOrderKits() as $orderProduct}
                                    <tr class="row-kits rowKits items-order row">
                                        <td class="frame-items frame-items-kit">
                                            <div class="title-h3 c_9">{lang('Комплект товаров', 'greyVision')}</div>
                            ...
                            <tfoot class="gen-info-price">
                                ...
                                {if $discount}
                                    <tr>
                                        <td colspan="3">
                                            <span class="s-t f_l">{lang('Начальная стоимость товаров','greyVision')}</span>
                                            <div class="f_r">
                                                <span class="price-new">
                                                    <span>
                                ...
                                <tr>
                                    <td colspan="3">
                                        <span class="s-t f_l">{lang('Cтоимость товаров','greyVision')}</span>
                                        <div class="f_r">
                                            <span class="price-new">
                                                <span>
                                ...
                                {if $deliveryMethod}
                                    <tr>
                                        <td colspan="3">
                                            <span class="s-t f_l">{lang('Доставка','greyVision')}:</span>
                                            <div class="f_r">
                                                {if !$deliveryMethod->getDeliverySumSpecified()}
                                                    ...
                                                    {if $cartPrice < $priceDelFreeFrom}
                                                        ...
                                                        {/if}
                                                    {else:}
                                                        <span class="text-el s-t">{lang('Бесплатно', 'greyVision')}</span>
                                                    {/if}
                                                {else:}
                                ...
                                {if $discount}
                                    <tr>
                                        <td colspan="3">
                                            <span class="s-t f_l">{lang('Ваша текущая скидка','greyVision')}:</span>
                                ...
                                {if $model->getGiftCertPrice() > 0}
                                    <tr>
                                        <td colspan="3">
                                            <span class="s-t">{lang('Подарочный сертификат','greyVision')}:</span>
                                            <span class="price-item f_r">
                ...
                <div class="footer-bask">
                    <div class="inside-padd">
                        ...
                        <div class="gen-sum-order clearfix">
                            <span class="title f_l">{lang('К оплате с учетом доставки','greyVision')}:</span>
