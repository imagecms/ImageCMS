<div class="drop drop-style drop-edit-wishlist">
    ...
    {foreach $wishlists as $key => $wishlist}
        <div class="drop-header">
            <div class="title">{lang('Редактировать:', 'greyVision')} {echo $wishlist[0]['title']}</div>
        </div>
                    ...
                    <form method="POST" action="{site_url('/wishlist/updateWL')}">
                            ...
                            <div class="frame-form-field check-public-drop">
                                <div class="lineForm">
                                    <select name="access" id="access">
                                        <option {if $wishlist['0']['access'] == 'shared'}selected="selected"{/if} value="shared">{lang('Коллективный', 'greyVision')}</option>
                                        <option {if $wishlist['0']['access'] == 'private'}selected="selected"{/if} value="private">{lang('Приватный', 'greyVision')}</option>
                                        <option {if $wishlist['0']['access'] == 'public'}selected="selected"{/if} value="public">{lang('Публичный', 'greyVision')}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        {if $wishlist[0][variant_id]}
                            {foreach $wishlist as $key => $w}
                                <div class="frame-label">
                                    <div class="title">
                                        <div>{lang('Коментарий к:', 'greyVision')}</div>
                        ...
                        <div class="frame-label">
                            ...
                            <div class="frame-form-field">
                                <div class="btn-def">
                                    <input type="submit" value="{lang('Сохранить', 'greyVision')}"/>
                                </div>
                            </div>
                        </div>
                        {form_csrf()}
