...
<table class="table-order table-order-view">
                        ...
                        <div class="description">
                            {if $item->getName() && trim($item->getName()) != trim($item->getSProducts()->getName())}
                                <span class="frame-variant-name">
                                    <span class="code text-el">{lang('Вариант','boxGreen')}</span>
                                    <span class="text-el">{echo trim($item->getName())}</span>
                                </span>
                            {/if}
                            {if $item->getNumber()}
                                <span class="frame-variant-code">{lang('Артикул','boxGreen')}  
                    ...
                    <td>
                        <div class="frame-frame-count">
                            <div class="frame-count">
                                <span class="plus-minus">{echo $item->quantity}</span><span class="text-el">{lang('шт.', 'boxGreen')}</span>
                            </div>
                        </div>
                    </td>
    ...
    <tfoot class="gen-info-price">
        ...
        {if $gift_val}
            <tr>
                <td colspan="3">
                    <span class="s-t f_l">{lang('Подарочный сертификат','boxGreen')}:</span>
                            ...
                            {if $gift_error}
                                <div class="msg">
                                    <div class="error">
                                        ...
                                        <span class="text-el">{lang('Неверный код подарочного сертификата', 'boxGreen')}</span>
                                    </div>
                                </div>
                            {else:}
                                <div class="btn-toggle-gift">
                                    <button type="button" class="d_l_3" data-drop="#gift" data-place="inherit" data-overlay-opacity="0">
                                        <span class="text-el">{lang('Ввести промо-код', 'boxGreen')}</span>
                                    </button>
                                </div>
                            {/if}
                            <div id="gift" class="{if !$gift_error}drop{/if} o_h">
                                <div class="btn-def f_r">
                                    <button type="button" id="giftButton">
                                        <span class="text-el">{lang('Применить', 'boxGreen')}</span>
                                    </button>
                                </div>
                                <div class="o_h">
