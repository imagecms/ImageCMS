...
var ShopFront = {
    Cart: {
        processBtnBuyCount: function(id, status, kit) {
            ...
            el.each(function() {
                ...
                if (status == 'change') {
                    Shop.Cart.getAmount(kit, id);
                    $(document).on('getAmount.Cart', function(e) {
                        el.closest(genObj.parentBtnBuy).find(genObj.frameCount).find('input').attr('value', e.datas);
                    })
                }
            });
