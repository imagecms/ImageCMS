...
(function($) {
    var methods = {
        ...
        droppable: function(drop) {
            ...
            if (drop === undefined)
                drop = this;
            drop.on('mousedown.drop', function(e) {
                ...
                $(document).on('selectstart.drop', function(e) {
                    e.preventDefault();
                });
                
                var condH = $(document).height() > wnd.height() && drop.data('place') == 'center';

                $this.on('mousemove.drop', function(e) {
                    var l = e.pageX - left,
                    t = e.pageY - top;
                    l = l < 0 ? 0 : l;
                    t = t < 0 ? 0 : t;
                    var addW = condH ? 17 : 0;
                    l = l + w + addW < wnd.width() ? l : wnd.width() - w - addW;
                    t = t + h < wnd.height() ? t : wnd.height() - h;
                    $this.css({
                        'left': l, 
                        'top': t
                    })
                    $this.on('mouseup', function(e) {
                        $this.css('cursor','default').off('mousemove.drop mouseup.drop');
                        $(document).off('selectstart.drop');
                    });
                })
            });
        }
