    ...
    var methods = {
        init: function(options) {
            if ($.existsN(this)) {
                ...
                frameChecks.find(wrapper).off('click.nstcheck').on('click.nstcheck', function(e) {
                    var $this = $(this),
    ...
    var methods = {
        init: function(options) {
            ...
            this.add($('[data-drop]')).each(function() {
                var el = $(this);
                el.off(ev || el.data('ev') + '.drop').on(ev || el.data('ev') + '.drop', function(e) {
                    methods.closeModal();
                    function confirmF() {
                        if ($.inArray(elSet.source, arrDrop) !== 0 || modal || always) {
                    ...
                    if (!$this.parent().hasClass(activeClass)) {
                        ...
                        methods.closeDrop($($this.data('drop')));
                    return false;
                });
            });
            return $(this);
        ...
        showDrop: function($this, e, set, isajax, data) {
            ...
            if (drop.hasClass(activeClass) && e.button !== undefined) {
                ...
                if (condOverlay) {
                    drop.data('dropOver').fadeIn($thisEOn / 2);
                    if (closeClick)
                    ...
                    if (data.modal || sel || condOverlay || data.place === 'noinherit') {
                        ...
                        if ($thisB !== undefined) {
                            ...
                            function _hide() {
                                ...
                                if (drop.data('forCenter'))
                                    drop.data('forCenter').stop(true, false).fadeOut(durOff);
                                if (drop.data('dropOver'))
        ...
        dropCenter: function(drop, start) {
            if (drop === undefined)
                drop = $(this);
