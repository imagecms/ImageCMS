...
<table class="table-order table-order-view">
                        ...
                        <div class="description">
                            {if $item->getName() && trim($item->getName()) != trim($item->getSProducts()->getName())}
                                <span class="frame-variant-name">
                                    <span class="code text-el">{lang('Вариант','box')}</span>
                                    <span class="text-el">{echo trim($item->getName())}</span>
                                </span>
                            {/if}
                            {if $item->getNumber()}
                                <span class="frame-variant-code">{lang('Артикул','box')}  
                    ...
                    <td>
                        <div class="frame-frame-count">
                            <div class="frame-count">
                                <span class="plus-minus">{echo $item->quantity}</span><span class="text-el">{lang('шт.', 'box')}</span>
                            </div>
                        </div>
                    </td>
    ...
    <tfoot class="gen-info-price">
        ...
        {if $gift_val}
            <tr>
                <td colspan="3">
                    <span class="s-t f_l">{lang('Подарочный сертификат','box')}:</span>
                            ...
                            {if $gift_error}
                                <div class="msg">
                                    <div class="error">
                                        ...
                                        <span class="text-el">{lang('Неверный код подарочного сертификата', 'box')}</span>
                                    </div>
                                </div>
                            {else:}
                                <div class="btn-toggle-gift">
                                    <button type="button" class="d_l_3" data-drop="#gift" data-place="inherit" data-overlay-opacity="0">
                                        <span class="text-el">{lang('Ввести промо-код', 'box')}</span>
                                    </button>
                                </div>
                            {/if}
                            <div id="gift" class="{if !$gift_error}drop{/if} o_h">
                                <div class="btn-def f_r">
                                    <button type="button" id="giftButton">
                                        <span class="text-el">{lang('Применить', 'box')}</span>
                                    </button>
                                </div>
                                <div class="o_h">
