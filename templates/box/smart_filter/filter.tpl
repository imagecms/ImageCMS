...
<div class="frames-checks-sliders">
    <div class="frame-slider" data-rel="sliders.slider1">
        <div class="inside-padd">
            <div class="title">{lang('Фильтр по цене', 'box')}</div>
            <div class="slider-cont">
                <noscript>{lang('Джаваскрипт не включен', 'box')}</noscript>
            ...
            <div class="form-cost number">
                <div class="t-a_j">
                    {/*may been delete*/}
                    <label>
                        <input type="text" class="minCost" data-title="{lang('только цифры', 'box')}" name="lp" value="{echo $curMin}" data-mins="{echo $minPrice}"/>
                    </label>
                    <label>
                        <input type="text" class="maxCost" data-title="{lang('только цифры', 'box')}" name="rp" value="{echo $curMax}" data-maxs="{echo $maxPrice}"/>
                    </label>
            ...
            <div class="inside-padd">
                <div class="title">
                    <span class="f-s_0">
                        ...
                        <span class="d_b">
                            <span class="text-el">{lang('Производитель', 'box')}</span>
                        </span>
                    </span>
                </div>
                        ...
                        {else:}
                            <div class="lineForm">
                                <select name="p[{echo $prop->property_id}][]" id="p{echo $prop->property_id}" onchange="Filter.changeSelectFilter(this)">
                                    <option id="none"  value=''>--{lang('Не выбрано', 'box')}--</option>
                                    {foreach $prop->possibleValues as $item}
    ...
    <div class="filter-foot">
        <div class="inside-padd t-a_c">
            <div class="btn-form">
                <button type="submit">
                    <span class="text-el">{lang('Фильтровать', 'box')}</span>
                </button>
            </div>
        </div>
    </div>
