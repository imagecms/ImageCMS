        ...
        <div class="js-empty empty {if count($items) == 0}d_b{/if}">
            <div class="f-s_0 title-cart without-crumbs">
                <div class="frame-title">
                    <h1 class="title">{lang('Оформление заказа','boxVertical')}</h1>
                </div>
            </div>
                        ...
                        {if !$is_logged_in}
                            <span class="old-buyer">
                                <button type="button" data-trigger="#loginButton">
                                    <span class="d_l text-el">{lang('Я уже тут покупал','boxVertical')}</span>
                                </button>
                            </span>
                        {/if}
                    </div>
                ...
                <form method="post" action="{$BASE_URL}shop/order/make_order" class="clearfix">
                    <div class="left-cart">
                        <div class="horizontal-form order-form big-title">
                            ...
                            <div class="groups-form">
                                <label>
                                    <span class="title">{lang('Имя получателя','boxVertical')}</span>
                                    ...
                                    <div class="frame-form-field">
                                        {if trim(ShopCore::app()->CustomFieldsHelper->setRequiredHtml('<span class="must">*</span>')->setPatternMain('pattern_custom_field_phone')->getOneCustomFieldsByName('addphone','order',$profile.id,'user')->asHtml()) != ''}
                                            <span class="f_r l-h_35">
                                                <button type="button" class="d_l_3" data-drop=".drop-add-phone" data-overlay-opacity="0" data-place="inherit">{lang('Еще один номер', 'boxVertical')}</button>
                                            </span>
                                        {/if}
                            ...
                            <div class="groups-form">
                                ...
                                <div class="frame-label" id="frameDelivery">
                                    <span class="title">{lang('Способ доставки','boxVertical')}</span>
                                    <div class="frame-form-field check-variant-delivery">
                                        {/* <div class="lineForm">
                                            <select id="method_deliv" name="deliveryMethodId">
                                                <option value="">{lang('--Выбирете способ доставки--', 'boxVertical')}</option>
                                ...
                                <div class="frame-label">
                                    <div class="frame-form-field">
                                        <button type="button" class="d_l_3 m-b_5" data-drop=".hidden-comment" data-place="inherit" data-overlay-opacity="0">{lang('Добавить комментарий к заказу', 'boxVertical')}</button>
                                ...
                                <div class="frame-payment p_r">
                                    <div id="framePaymentMethod">
                                        <div class="frame-label">
                                            <span class="title">{lang('Оплата','boxVertical')}:</span>
                                            <div class="frame-form-field" style="padding-top: 6px;">
                                                <div class="help-block">{lang('Выберите доставку', 'boxVertical')}</div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="frame-label">
                                    ...
                                    <span class="frame-form-field">
                                        <div class="btn-buy btn-buy-p btn-buy-pp">
                                            <input type="submit" value="{lang('Подтвердить заказ','boxVertical')}" id="submitOrder"/>
                                        </div>
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="right-cart">
                        <div class="frame-bask frame-bask-order">
                            <div class="frame-title clearfix">
                                <div class="title f_l">{lang('Мой заказ', 'boxVertical')}</div>
                                <div class="f_r">
                                    <button type="button" class="d_l_3 editCart">{lang('Редактировать', 'boxVertical')}</button>
                                </div>
                            </div>
