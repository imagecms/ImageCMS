...
<div class="frame-bask frameBask p_r">
    ...
    {if $totalItems > 0}
        <div class="drop-header">
            <div class="title bask"><span>{lang('Корзина товаров','boxVertical')}</span></div>
        </div>
                            ...
                            {if $item->instance === "SProducts"}
                                <tr data-id="{echo $item->getId()}" class="items items-bask cart-product">
                                        ...
                                        <div class="description">
                                            {if $item->getName() && trim($item->getName()) != trim($item->getSProducts()->getName())}
                                                <span class="frame-variant-name">
                                                    <span class="code">{lang('Вариант','boxVertical')}:</span>
                                    ...
                                    <td class="frame-count frameCount">
                                        <div class="number js-number" data-title="{lang('Количество на складе','boxVertical')} {echo $item->getStock()}">
                                            <input type="text" value="{echo $item->quantity}" class="plusMinus plus-minus" id="inputChange{echo $item->getId()}" data-id="{echo $item->getId()}" data-title="{lang('Только цифры','boxVertical')}" data-min="1" data-max="{echo $item->getStock()}"/>
                                        </div>
                                        <span class="s-t f-s_13">{lang('шт.', 'boxVertical')}</span>
                                    </td>
                            ...
                            {else:}
                                <tr class="row-kits" data-id="{echo $item->getId()}">
                                    ...
                                    <td class="frame-count">
                                        <div class="number js-number" data-title="{lang('Количество на складе','boxVertical')} {echo $item->getStock()}">
                                            <input type="text" value="{echo $item->quantity}" class="plusMinus plus-minus" id="inputChange{echo $item->getId()}" data-id="{echo $item->getId()}" data-kit="1" data-title="{lang('Только цифры','boxVertical')}" data-min="1" data-max="{echo $item->getStock()}"/>
                                        </div>
                                        <span class="f-s_13 s-t">{lang('шт.', 'boxVertical')}</span>
                                    </td>
                                    <td class="frame-cur-sum-price">
                                        <div class="frame-prices f-s_0">
                                            {if ShopCore::app()->SCurrencyHelper->convert($item->originPrice) != ShopCore::app()->SCurrencyHelper->convert($item->price)}
                ...
                <div class="clearfix">
                    <div class="f_l isCart">
                        <button type="button" data-closed="closed-js">
                            <span class="text-el"><span class="f-s_14 ref2">←</span> <span class="d_l_3">{lang('Вернуться к оформлению','boxVertical')}</span></span>
                        </button>
                    </div>
                    <div class="f_r">
                        {if $discount_val}
                            <span class="frame-discount">
                                <span class="s-t">{lang('Ваша скидка','boxVertical')}:</span>
                        ...
                        {/if}
                        <span class="c_6 f-s_13">{lang('Сумма товаров','boxVertical')}:</span>
            ...
            <div class="content-frame-foot notCart">
                <div class="clearfix inside-padd">
                    <div class="f_l" style="margin-top: 9px;">
                        <button type="button" data-closed="closed-js">
                            <span class="text-el"><span class="f-s_14 ref2">←</span> <span class="d_l_3">{lang('Продолжить покупки','boxVertical')}</span></span>
                        </button>
                    </div>
                    <div class="btn-buy btn-buy-p btn-buy-pp f_r">
                        <a href="{shop_url('cart')}">
                            <span class="text-el">{lang('Оформить заказ','boxVertical')}</span>
                        </a>
                    </div>
                </div>
            </div>
    ...
    {else:}
        <div class="drop-header">
            <div class="title">{lang('Ваша корзина','boxVertical')} <span class="add-info">{lang('пуста','boxVertical')}</span></div>
        </div>
        <div class="drop-content is-empty">
            <div class="inside-padd">
                <div class="msg f-s_0">
                    <div class="success"><span class="icon_info"></span><span class="text-el">{lang('Вы удалили все элементы из корзины','boxVertical')}</span></div>
                </div>
                <div class="notCart">
                    <button type="button" data-closed="closed-js">
                        <span class="text-el"><span class="f-s_14 ref2">←</span> <span class="d_l_3">{lang('Продолжить покупки','boxVertical')}</span></span>
