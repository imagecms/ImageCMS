...
<form method="post" action="{site_url('admin/components/init_window/template_manager/updateComponent')}/{echo $handler}" id="component_{echo $handler}_form">
    ...
    <table class="frame-level-menu frame_level table table-bordered table-condensed products_table">
        <thead>
            <tr>
                <td>{lang('Category', 'boxVertical')}</td>
                <td class="span2" style="padding-left: 5px;text-align: center;">{lang('Show in column', 'boxVertical')}</td>
            </tr>
        </thead>
        <tbody>
        <select name="openLevels">
            <option {if $openLevels == '2'}selected="selected"{/if} value="2">{lang('Open second level', 'boxVertical')}</option>
            <option {if $openLevels == 'all'}selected="selected"{/if}value="all">{lang('Open all levels', 'boxVertical')}</option>
        </select>
        ...
        {foreach $categoriesT as $key => $category}
            ...
            <tr>
                <td>
                    <div class="title lev">
                        {if $children}
                            <button type="button" class="btn btn-small my_btn_s" data-rel="tooltip" data-placement="top" data-original-title="{lang('Toggle this category', 'boxVertical')}" style="display: none;" onclick="hideSubCategory(this)">
                                <i class="my_icon icon-minus"></i>
                            </button>
                            <button type="button" class="btn btn-small my_btn_s btn-primary" data-rel="tooltip" data-placement="top" data-original-title="{lang('Expand Category', 'boxVertical')}" onclick="showSubCategory(this)">
                                <i class="my_icon icon-plus"></i>
                            </button>
                        {/if}
                        <a href="/admin/components/run/shop/categories/edit/{echo $category->id}" class="pjax" data-rel="tooltip" data-placement="top" data-original-title="{lang('Editing category', 'boxVertical')}">
                            {echo $category->name}
                        </a>
                    </div>
                </td>
...
<button type="button" class="btn btn-small action_on formSubmit btn-primary cattegoryColumnSaveButtonMod" data-form="#component_{echo $handler}_form" data-action="close">
    <i class="icon-ok icon-white"></i>{lang('Save', 'boxVertical')}
</button>
{literal}
    <script>
