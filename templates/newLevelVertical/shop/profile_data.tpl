            ...
            <div class="content">
                <ul class="items items-info-discount">
                    ...
                    {if $discount_comul_next}
                        <li class="inside-padd">
                            <div>{lang('Для следующей скидки','newLevelVertical')} {echo $discount_comul_next['value']}{if  $discount_comul_next['type_value'] == 1}%{else:}{$CS}{/if}</b> {lang('осталось','newLevelVertical')}</div>
                            <div>{lang('cделать покупки на сумму','newLevelVertical')}: <b>{echo $discount_comul_next['begin_value'] - $profile->getamout()} {$CS}</b></div>
                        </li>
                    {/if}
                    {if  $discount['comulativ']}
                        <li class="inside-padd">
