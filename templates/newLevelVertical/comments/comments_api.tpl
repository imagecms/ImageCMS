{if $can_comment == 1 AND !$is_logged_in}
    <span class="title-comment"><b>{sprintf(lang('Пожалуйста, войдите для комментирования', 'newLevelVertical'), site_url($modules.auth))}</b></span>
...
<div class="comments" id="comments">
    ...
    <div class="title-comment">{lang('Отзывы покупателей', 'newLevelVertical')} {if $showButton && $can_comment === 0 || $showButton && $is_logged_in}<button class="d_l_1" data-drop=".comments-main-form" data-place="inherit" data-overlay-opacity="0" data-after="Comments.toComment">{lang('Оставить свой отзыв', 'newLevelVertical')}</button>{/if}</div>
        {if $comments_arr}
        <div class="frame-list-comments">
                    ...
                    <li>
                        ...
                        <div class="clearfix global-frame-comment-sub1">
                            ...
                            <div class="frame-mark">
                                {if $comment.rate != 0}
                                    <div class="mark-pr">
                                        <span class="title">{lang('Оценка товара','newLevelVertical')}:</span>
                                ...
                                <div class="func-button-comment">
                                    <span class="s-t">{lang('Отзыв полезен?','newLevelVertical')}</span>
                                    <span class="btn like">
                                        <button type="button" class="usefullyes" data-comid="{echo $comment.id}">
                                            <span class="icon_like"></span>
                                            <span class="text-el d_l_1">{lang('Да','newLevelVertical')} <span class="yesholder{$comment.id}">({echo $comment.like})</span></span>
                                        </button>
                                    </span>
                                    <span class="btn dis-like">
                                        <button type="button" class="usefullno" data-comid="{echo $comment.id}">
                                            <span class="icon_dislike"></span>
                                            <span class="text-el d_l_1">{lang('Нет','newLevelVertical')} <span class="noholder{$comment.id}">({echo $comment.disslike})</span></span>
                                        </button>
                                    </span>
                                </div>
                            </div>
                            <div class="frame-comment-sub1">
                                <div class="frame-comment">
                                    ...
                                    {if $comment.text_plus != Null}
                                        <p>
                                            <b>{lang('Да', 'newLevelVertical')}</b><br>
                                            {$comment.text_plus}
                                        </p>
                                    {/if}
                                    {if $comment.text_minus != Null}
                                        <p>
                                            <b>{lang('Нет', 'newLevelVertical')}</b><br>
                                            {$comment.text_minus}
                                        </p>
                                    {/if}
                                </div>
                        ...
                        <div class="btn-all-comments">
                            <button type="button"><span class="text-el" data-hide='<span class="d_l_1">{lang('Скрыть','newLevelVertical')}</span> ↑' data-show='<span class="d_l_1">{lang('Смотреть все ответы','newLevelVertical')}</span> ↓'></span></button>
                        </div>
                    </li>
                {/foreach}
            </ul>
            {if $CI->core->core_data['data_type'] != 'page'}
                <button class="t-d_n f-s_0 s-all-d ref d_n_" data-trigger="[data-href='#comment']" data-scroll="true">
                    ...
                    <span class="text-el">{lang('Смотреть все ответы','newLevelVertical')}</span>
                </button>
            {/if}
        </div>
    {/if}
        ...
        <div class="drop comments-main-form {if !$comments_arr}noComments{/if} {if $visibleMainForm || $visibleMainForm == NULL}active inherit{/if}" {if $visibleMainForm}style="display: block;"{/if}>
            <div class="frame-comments layout-highlight">
                <div class="title-default title-comment">
                    <div class="title">{lang('Оставить комментарий','newLevelVertical')}</div>
                </div>
                    ...
                    <div class="inside-padd">
                        <form method="post">
                            ...
                            {if !$is_logged_in}
                                <div class="clearfix">
                                    ...
                                    <label style="width: 45%;margin-left: 10%;float: left;">
                                        <span class="title">{lang('Ваш email:', 'newLevelVertical')}</span>
                                        <span class="frame-form-field">
                            ...
                            <div class="frame-label">
                                <span class="title f_l t-a_l">{lang('Ваша оценка:', 'newLevelVertical')}</span>
                            ...
                            <div class="frame-label">
                                <span class="frame-form-field">
                                    <div class="btn-form">
                                        <input type="submit" value="{lang('Комментировать', 'newLevelVertical')}" onclick="Comments.post(this, {literal}{'visibleMainForm': '1'}{/literal}, '.mainPlace')"/>
                                    </div>
                                </span>
                            </div>
                        </form>
    ...
    <div class="frame-drop-comment" data-rel="whoCloneAddPaste">
        <div class="form-comment layout-highlight frame-comments">
            <div class="title-default title-comment">
                <div class="title">{lang('Ваш ответ','newLevelVertical')}</div>
            </div>
            <div class="inside-padd">
                <form>
                    ...
                    {/if}
                    <label>
                        <span class="title">{lang('Текст ответа:','newLevelVertical')}</span>
                        <span class="frame-form-field">
                    ...
                    <div class="frame-label">
                        <span class="frame-form-field">
                            ...
                            <span class="btn-form">
                                <input type="submit" value="{lang('Комментировать', 'newLevelVertical')}" onclick="Comments.post(this, {if $visibleMainForm}{literal}{'visibleMainForm': '1'}{/literal}{else:}{literal}{'visibleMainForm': '0'}{/literal}{/if})"/>
                            </span>
                        </span>
                    </div>
                </form>
...
<div class="d_n" id="useModeration">
    <div class="usemoderation">
        <div class="msg">
            <div class="success">
                {lang('Ваш комментарий будет опубликован после модерации администратором','newLevelVertical')}
            </div>
        </div>
    </div>
</div>
