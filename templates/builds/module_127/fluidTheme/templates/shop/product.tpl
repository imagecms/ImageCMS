...
<div class="frame-inside page-product">
    <div class="container">
        ...
        <div class="clearfix item-product {if $model->firstVariant->getStock() == 0} not-avail{else:}{if $inCartFV} in-cart{else:} to-cart{/if}{/if}">
                ...
                <div class="left-product-left {if $sizeAddImg > 0}is-add{/if}">
                    <div class="inside">
                        ...
                        <a rel="position: 'xBlock'" onclick="return false;" href="{echo $model->firstVariant->getLargePhoto()}" class="frame-photo-title photoProduct cloud-zoom" id="photoProduct" title="{echo ShopCore::encode($model->getName())}" data-drop="#photo" data-start="Product.initDrop" data-scroll-content="false">
                            <span class="photo-block">
                                <span class="helper"></span>
                                <img src="{echo $model->firstVariant->getMainPhoto()}" alt="{echo ShopCore::encode($model->getName())} - {echo $model->getId()}" title="{echo ShopCore::encode($model->getName())} - {echo $model->getId()}" class="vImgPr"/>
                                {$discount = 0}
                                {if $hasDiscounts}
                    ...
                    <div class="t-a_j">
                        <div class="btn-additional-s_c2 mq-w-480 mq-inline-block mq-min showComment">
                            <button type="button">
                                <span class="text-el" data-hide='<span class="d_l">{lang('Скрыть отзывы','newLevel')}</span> <span class="icon-show-part up"></span>' data-show='<span class="d_l">{lang('Смотреть отзывы','newLevel')}</span> <span class="icon-show-part"></span>'></span>
                            </button>
                        </div>
                        <div class="btn-additional-s_c2 mq-w-480 mq-inline-block mq-min showCommentForm">
                            <button type="button">
                                <span class="text-el" data-hide='<span class="d_l">{lang('Скрыть форму','newLevel')}</span> <span class="icon-show-part up"></span>' data-show='<span class="d_l">{lang('Оставить отзыв','newLevel')}</span> <span class="icon-show-part"></span>'></span>
                            </button>
                        </div>
                    </div>
                                            ...
                                            <li class="globalFrameProduct{if $inCart} in-cart{else:} to-cart{/if}">
                                                ...
                                                <div class="complect-gen-sum">
                                                    <div class="frame-gen-price-buy-complect">
                                                        ...
                                                        <form method="POST" action="/shop/cart/addKit/{echo $kitProducts->getId()}" class="d_i-b v-a_m">
                                                            <div class="btn-buy btn-cart{if !$inCart} d_n{/if}">
                                                                <button 
                                                                    type="button"
                    ...
                    <div class="right-view-product">
                        ...
                        {if $accessories}
                            <div class="accessories">
                                ...
                                <div class="inside-padd">
                                    <ul class="items items-default items-product accessories-height">
                                        {$CI->load->module('new_level')->OPI($accessories, array('opi_defaultItem'=>true))}
                                    </ul>
                                    <div class="btn-additional-s_c2">
                                        <button type="button">
                                            <span class="text-el" data-hide='<span class="d_l">{lang('Скрыть','newLevel')}</span> <span class="icon-show-part up"></span>' data-show='<span class="d_l">{lang('Показать еще аксессуаров','newLevel')}</span> <span class="icon-show-part"></span>'></span>
                                        </button>
                                    </div>
                                </div>
                            </div>
