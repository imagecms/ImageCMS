...
<div class="di_b">
    <span class="comment_ajax_refer b-r_4 visible">
        ...
        <a href="#" class="t-d_n"><span class="js">{lang("leave a comment")}</span><span class="blue_arrow"></span></a>

        {if $is_logged_in}
            {lang("You are logged in as")} {$username}
        {else:}
...
<form action="" method="post" class="comment_form clearfix">
    ...
    <label>{lang("Text comments")}<span style="color:red;">*</span>
        ...
        <div style="padding-bottom:4px;">
            <p class="clear">
                {if $captcha_type == 'captcha'}
                    <label for="captcha" style="width:140px;" class="left">{lang("Code protection")}<span style="color:red;">*</span></label>
    ...
    <label class="buttons button_middle_blue f_l">
        <input type="submit" value="{lang("leave a comment")}" onclick="json()"/>
    </label>
    ...
    <ul class="comments" style="width:100%">
        {foreach $comments_arr as $comment}
            <li>
                ...
                <div class="c_9 f-s_11">{lang("On")} {date('d-m-Y H:i', $comment.date)}</div>
                <p>{$comment.text}</p>
                ...
                <div class="di_b">
                    <span class="comment_ajax_refer b-r_4 visible">
                        ...
                        {if $can_comment == 0 OR $is_logged_in}
                        <a href="#" class="t-d_n">
                            <span class="js">{lang("Answer")}</span>

                            <span class="blue_arrow"></span></a>

                        {/if}

                        {lang("Review was helpful?")}
                        <span></span>

                        <span class="usefullyes" data-comid="{echo $comment.id}"><span class="js">{lang("Yes")}</span></span><span id="yesholder{$comment.id}">({echo $comment.like})</span>/
                        <span class="usefullno" data-comid="{echo $comment.id}"><span class="js">{lang("No")}</span></span><span id="noholder{$comment.id}">({echo $comment.disslike})</span>
                    </span>
                </div>
            ...
            {/if}
                {foreach $comment_ch as $com_ch}
                    ...
                    <li style="padding-left: 50px">
                        ...
                        <div class="c_9 f-s_11">{lang("On")} {date('d-m-Y H:i', $com_ch.date)}</div>
                        <p>{$com_ch.text}</p>
                    </li>
                {/if}
            {/foreach}
