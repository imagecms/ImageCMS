...
<div class="content">
    <div class="center">
        <div class="filter">
            <div class="title padding_filter">{lang("Found in categories")}:</div>
        ...
        <div class="catalog_content">
            <div class="catalog_frame">
                <div class="box_title clearfix">
                    <div class="f-s_24 f_l">
                        {if !empty(ShopCore::$_GET['text'])}
                            {lang("You searched for")}: "<span class="highlight">{encode($_GET['text'])}</span>"
                        {/if}
                        <span class="count_search">
                            ({$totalProducts}) {echo SStringHelper::Pluralize($totalProducts, array(lang("product"), lang("product"), lang("product")))}
                        </span>
                    </div>
                </div>
                <div class="c_b"></div>
                                ...
                                <div class="func_description">
                                    ...
                                    <div class="f-s_0">
                                        {if $p->firstVariant->getNumber()}
                                            <span id="code{echo $p->getId()}" class="code">{lang("ID")} {echo ShopCore::encode($p->firstVariant->getNumber())}</span>
                                        {/if}
                                        <div class="star_rating">
                                            {$CI->load->module('star_rating')->show_star_rating($p)}
                                        </div>
                                        <a href="#" class="response">{echo $p->totalComments()} {echo SStringHelper::Pluralize($p->totalComments(), array(lang("review"), lang("reviews"), lang("review")))}</a>
                                    </div>
                                    ...
                                    {if $p->getShortDescription()!=""}
                                        <p class="c_b">
                                            ...
                                            <a href="{shop_url('product/'.$p->getUrl())}" class="t-d_n"><span class="t-d_u">{lang("More")}</span> →</a>
                                        </p>
                                    {/if}
                                </div>
                            </li>
                ...
                {else:}
                    <p>
                        {echo ShopCore::t(lang("Your search did not match"))}.
                    </p>
                {/if}
            </div>
        </div>
