...
<div class="content">
    ...
    {if $CI->session->flashdata('makeOrder') === true}<div style="padding:10px;border: 1px #f5f5dc solid;">{lang("Thank you for your order.")}</div>{/if}
    <div class="foot_cleaner">
        <ul class="info_curr_buy">
            <li>
                <span>{lang("Paid")}:</span>
                <b>{if $model->getPaid() == true} {lang("Yes")}{else:}{lang("No")}{/if}</b>
            </li>
            <li>
    ...
    <div class="order-cleaner">
        <table class="cleaner_table" cellspacing="0">
            <caption>{lang("Order")} №{echo $model->getId()}</caption>
            <colgroup>
                        ...
                        {if $item->getIsMain()}
                            <tr>
                                ...
                                <td rowspan="{echo $kits[$item->getKitId()]['total']}">
                                    {echo $item->getQuantity()} {lang("Pcs")}.
                                </td>
                                <td rowspan="{echo $kits[$item->getKitId()]['total']}">
