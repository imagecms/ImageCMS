{if $_GET['brand'] != "" || $_GET['p'] != "" || ($_GET['lp'] && $_GET['lp'] != $minPrice) || ($_GET['rp'] && $_GET['rp'] != $maxPrice)}
    <div class="frame-check-filter">
        <div class="inside-padd">
            <div class="title">{echo count($products)} {echo SStringHelper::Pluralize(count($products), array(lang('товар', 'inTime'),lang('товара', 'inTime'),lang('товаров', 'inTime')))} {lang('с фильтрами', 'inTime')}:</div>
            <ul class="list-check-filter">
                {if $curMin != $minPrice || $curMax != $maxPrice}
                    <li class="clear-slider" data-rel="sliders.slider1"><button type="button"><span class="icon_times icon_remove_filter f_l"></span><span class="name-check-filter">{lang('Цена от', 'inTime')} {echo $_GET['lp']} {lang('до', 'inTime')} {echo $_GET['rp']} <span class="cur">{$CS}</span></></button></li>
                    {/if}
                    {if count($brands) > 0}
            ...
            <div class="foot-check-filter">
                <button type="button" onclick="location.href = location.origin + location.pathname" class="btn-reset-filter">
                    ...
                    <span class="text-el d_l_2">{lang('Сбросить все параметры', 'inTime')}</span>
                </button>
            </div>
        </div>
    </div>
