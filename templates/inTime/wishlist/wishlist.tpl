    ...
    <div class="container">
        <div class="f-s_0 title-cart without-crumbs">
            <div class="frame-title">
                <h1 class="title">{lang('Список желаний','inTime')}</h1>
            </div>
        </div>
        {if $errors}
            ...
            <div id="list-products" style="display: block;">
                <div class="frame-button-add-wish-list">
                    <div class="btn-buy">
                        <button type="button" data-drop=".drop-add-wishlist" data-place="inherit" data-overlay-opacity="0" data-effect-on="slideDown" data-effect-off="slideUp">
                            ...
                            <span class="text-el">{lang('Создать новый список','inTime')}</span>
                        </button>
                    </div>
                    <span class="help-block">{lang('В список избранных вы можете отложить понравившиеся товары, также показать список друзьям', 'inTime')}</span>
                </div>
                <div class="drop drop-style-2 drop-add-wishlist">
                    <div class="drop-header">
                        <div class="title">{lang('Создание списка избранных товаров','inTime')}</div>
                    </div>
                            ...
                            <div class="horizontal-form big-title">
                                <form method="POST" action="{site_url('/wishlist/wishlistApi/createWishList')}">
                                    ...
                                    <div class="frame-label">
                                        <span class="title">{lang('Доступность:','inTime')}</span>
                                        <div class="frame-form-field check-public">
                                            <div class="lineForm">
                                                <select name="wlTypes" id="wlTypes">
                                                    <option value="shared">{lang('Коллективный', 'inTime')}</option>
                                                    <option value="public">{lang('Публичный', 'inTime')}</option>
                                                    <option value="private">{lang('Приватный', 'inTime')}</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    <label>
                                        <span class="title">{lang('Название списка:','inTime')}</span>
                                        <span class="frame-form-field">
                                            <input type="text" value="" name="wishListName"/>
                                        </span>
                                    </label>
                                    <label>
                                        <span class="title">{lang('Описание:','inTime')}</span>
                                        <span class="frame-form-field">
                                    ...
                                    {else:}
                                        <div class="msg layout-highlight layout-highlight-msg">
                                            <div class="info">
                                                ...
                                                <span class="text-el">{lang('Список пуст','inTime')}</span>
                                            </div>
                                        </div>
                                    {/if}
                                </div>
                                    ...
                                    <div class="funcs-buttons-wishlist d_i-b">
                                        <div class="btn-edit-WL">
                                            <button
                                                ...
                                                data-always="true"
                                                >
                                                <span class="d_l_1 text-el">{lang('Редактировать список','inTime')}</span>
                                            </button>
                                        </div>
                                        <div class="btn-remove-WL">
                                            <button
                                                ...
                                                <span class="text-el d_l_1">{lang('Удалить список','inTime')}</span>
                                            </button>
                                        </div>
                                    </div>
                                    <div class="d_i-b m-r_10">
                                        <b>{lang('Доступность:','inTime')}</b>
                                        {if $wishlist[0][access] == 'private'}
                                            <span class="s_t">{lang('Приватный', 'inTime')}</span>
                                        {/if}
                                        {if $wishlist[0][access] == 'public'}
                                            <span class="s_t">{lang('Публичный', 'inTime')}</span>
                                        {/if}
                                        {if $wishlist[0][access] == 'shared'}
                                            <span class="s_t">{lang('Коллективный', 'inTime')}</span>
                                        {/if}
                                    </div>
                                    {if $wishlist[0]['access'] == 'shared' || $wishlist[0]['access'] == 'public'}
                                        <div class="btn-form btn-send-wishlist d_i-b m-r_10">
                                            <button type="button" data-drop=".drop-sendemail" title="{lang('Поделится с другом','inTime')}" data-source="{echo site_url('wishlist/wishlistApi/renderEmail/' . $wishlist[0][wish_list_id])}">
                ...
                {else:}
                    <div class="msg layout-highlight layout-highlight-msg">
                        <div class="info">
                            <span class="icon_info"></span>
                            <span class="text-el">{lang('Список Желания пуст','inTime')}</span>
                        </div>
                    </div>
                {/if}
            </div>
