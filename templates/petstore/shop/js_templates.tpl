...
<a href="{shop_url('search')}?text={literal}<%- items.queryString %>" {/literal} class="f-s_0 a_gray">
<span class="icon_show_all"></span><span class="text-el t-d_u">{lang('Посмотреть все результаты','newLevel')}</span><span> →</span>
</a>
</div>{literal}
<!-- End. Show link  -->
<% } else {%>    
    ...
    <%if (item.origprice) { %>
        ...
        <%if (nextCsCond){%>
            ...
            <div class="drop drop-style" id="dropAuth">
                ...
                <div class="drop-header t-a_c">
                    <span class="title error"><span class = "icon_info"></span>{lang('предупреждение', 'newLevel')}</span>
                   <div class = "msg js-msg m-t_10">
                        <div class="text-el c_6"> {lang('В список желаний могут добавлять только авторизированные пользователи', 'newLevel')} </div>
                </div>
            </div>
            <div class="drop-content t-a_c" style="width: 320px;min-height: 0;">
                <div class="inside-padd">
