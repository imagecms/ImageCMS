...
{if $_GET['brand'] != "" || $_GET['p'] != "" || ($_GET['lp'] && $_GET['lp'] != $minPrice) || ($_GET['rp'] && $_GET['rp'] != $maxPrice)}
 <div class="check-filter frame-group-checks frames-checks-sliders" data-rel="dropDown" id="dropDown111">
        ...
        <div class="title">
            <span class="f-s_0">
                ...
                <span class="d_b">
                    <span class="text-el">{lang('выбранные фильтры', 'newLevel')}</span>
                </span>
            </span>
        </div>
<div>  
            ...
            <div class="foot-check-filter">
                <button type="button" onclick="location.href = '{site_url($CI->uri->uri_string())}'" class="btn-reset-filter red">
                    <span class="text-el">{lang('Сбросить все фильтры', 'newLevel')}</span>
                </button>
            </div>
             </div>
        </div>
