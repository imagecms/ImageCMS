...
<script type="text/template" id="cartPopupTemplate">
    ...
    <div class="fancy fancy_cleaner frame_head_content">
        <div class="header_title"><%- lang.yourCart %>
        </div>
        <button type="button" class="icon-times-enter" data-closed="closed-js" onclick="togglePopupCart()"></button>
        <div class="drop-content">
            <div class="inside_padd">

                <div class="msg" style="display: none;"><div class="success"><%- lang.yourCartIsEmpty %>.</div></div>
                    ...
                    <tbody>
                        ...
                        <tr data-prodid="<%- item.id %>" data-varid="<%- item.vId %>" id="popupProduct_<%- item.id+'_'+item.vId %>">
                            ...
                            <td class="description">
                                ...
                                <div>
                                    <%if(item.vname){ %><span class="frame_variant_name">{/literal}{lang('Вариант', 'webinger')} {literal} <span class="code">(<%- item.vname%>)</span></span> <% } %>
                                    <%if (item.number) { %><span class="frame_number">{/literal}{lang('Арктикул', 'webinger')} {literal} <span class="code">(<%-item.number %>)</span></span> <% } %>
                                </div>
                            ...
                            <td>
                                <div class="frame_count number d_i-b v-a_m" data-title="<%- lang.wairhouseCount %><%-  item.maxcount%>">
                                    ...
                                    <input type="text" value="<%- item.count %>" data-rel="plusminus" data-title="<%- lang.onlyNumeric %>" data-min="1" <% if (item.maxcount) { %> data-max="<%-item.maxcount%>" <% } %> />
                                </div>
                                <span class="v-a_m countOrCompl"><%-pcs%></span>
                            </td>
                            <td>
                                <span class="d_b"><%- lang.sum %>: </span>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td rowspan="<%- names.length %>">
                                                <div class="frame_count number d_i-b v-a_m" data-title="<%-  lang.wairhouseCount %> <%-item.maxcount%>">
                                                    ...
                                                    <input type="text" data-min="1" data-title="<%- lang.onlyNumeric %>" data-rel="plusminus" value="<%-item.count%>" <% if (item.maxcount) { %> data-max="<%-item.maxcount%>" <% } %> />
                                                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="description">
                                                ...
                                                <div>
                                                    <%if(item.vname){ %><span class="frame_variant_name">{/literal}{lang('Вариант', 'webinger')} {literal} <span class="code">(<%- item.vname%>)</span></span> <% } %>
                                                    <%if (item.number) { %><span class="frame_number">{/literal}{lang('Артикул', 'webinger')} {literal} <span class="code">(<%-item.number %>)</span></span> <% } %>
                                                </div>
                                                <div class="price price_f-s_16">
                                                    <span class="first_cash"><span class="f-w_b"><%-prices[i]%></span> <%-curr%></span>
                                                </div>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td colspan="4" class="t-a_r">
                                <a href="#"  onclick="renderOrderDetails(); togglePopupCart(); return false;" class="btn btn_cart v-a_m m-r_30">{/literal}{lang('Закрыть','webinger')}{literal}</a>
                            </td>
                            <% } else { %>
                            <td colspan="4">
                                <a href="/shop/cart" class="btn btn_cart m-r_30 f_r"><%- lang.makeOrder %></a>
                                <button type="button" onclick="togglePopupCart()" class="d_l_b w-s_n-w f_l">← <%- lang.continueShopping %></button>
                            </td>

                            <% } %>
                            <td style="padding-left: 0;">
                                <div class="t-a_l d_i-b v-a_m">
                                    <span class="d_b"><%- lang.inTotal %>:</span>
