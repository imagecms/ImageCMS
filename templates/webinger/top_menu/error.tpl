...
{/literal}
<div class="error">
	<h3 align="center">{lang('Внимание', 'webinger')}!</h3>
	<p>{lang('Не обнаружен необхо димый файл шаблона для меню ', 'webinger')}<b>{$menu}</b></p>
	<ul>
	{foreach $errors as $e}
	...
	<p>
		{if $tpl_folder}
		{lang('Проверьте наличие файла или измените настройки меню ', 'webinger')}<b>{$menu}</b>
		{else:}
		{lang('Переустановите модуль меню', 'webinger')}
		{/if}
	</p>
</div>	