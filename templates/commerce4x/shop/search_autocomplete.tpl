        ...
        <div class="inside_padd">
            ...
            <div class="btn-form">{/literal}
                <a href="{shop_url('search')}?text={literal}<%- items.queryString %>" {/literal} class="f-s_0">
                    <span class="icon-show-all"></span><span class="text-el">{lang("View all results")}</span>
                </a>
            </div>{literal}
            <!-- End. Show link  -->
            <% } else {%>    
        {/literal}<div class="p_lr_10">{echo ShopCore::t(lang("Your search did not match"))}</div>{literal}
            <% }%>
        </div>
    </script>
{/literal}